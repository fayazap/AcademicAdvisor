...
df = pd.read_csv('nirfnext.csv', encoding='latin-1')

# Rename the column 'ï»¿Course' to 'Course'
...
try:
    for index, row in df.iloc[last_successful_index:].iterrows():
        ...
        prompt2 = row['City']
        prompt3 = row['State']
        prompt_text = f"list the bachelor courses provided by the {prompt} college in the {prompt2} city in the state {prompt3} that is courses which a 12th pass out can be joined. Just list them"
        ...
        df.at[index, 'Courses'] = generated_text
